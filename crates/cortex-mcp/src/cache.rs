...
pub struct L1Cache {
    ...
    store: DashMap<String, Box<dyn std::any::Any + Send + Sync>>,
...
impl L1Cache {
    ...
    pub fn get<T: Clone + 'static>(&self, key: &str, repo_revision: &str) -> Option<T> {
        ...
        let any_val: &(dyn std::any::Any + Send + Sync) = &**entry.value();
        any_val.downcast_ref::<T>().cloned()
    }

    /// Put a value into the cache
    pub fn put<T: Send + Sync + 'static>(&self, key: &str, value: T, repo_revision: &str) {
    ...
    pub fn put_with_ttl<T: Send + Sync + 'static>(
        &self,
        key: &str,
        value: T,
        ttl: Duration,
    ...
    where
        F: FnMut(&str, &Box<dyn std::any::Any + Send + Sync>) -> bool,
    {
...
impl CacheHierarchy {
    ...
    pub fn put<T: Serialize + Clone + Send + Sync + 'static>(&self, key: &str, value: T, repo_revision: &str) {
    ...
    pub fn put_with_ttls<T: Serialize + Clone + Send + Sync + 'static>(
        &self,
        key: &str,
        value: T,
        repo_revision: &str,
