...
All MCP tools are **enabled by default**. Disable a surface with
`CORTEX_FLAG_MCP_<NAME>_ENABLED=0` (for example `CORTEX_FLAG_MCP_IMPACT_GRAPH_ENABLED=0`).
`CORTEX_MCP_PROFILE=strict` still disables memory, vector write, and context capsule on top of env.

Repeatable `--enable` flags on `cortex mcp start` force named groups on when you previously
...
Useful `--enable` values include `memory`, `memory-read`, `memory-write`,
`context-capsule`, `impact-graph`, `logic-flow`, `index-status`, `skeleton`,
`workspace-setup`, `manage-codecortex`, and `lsp-ingest`.
